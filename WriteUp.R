...
if (length(which(M>0.8,arr.ind = T)) == 0){
  train = train_raw
  test = test_raw
...
#model training: I choose to use the random forest algorithm as decision trees
# seem to me more suited for multi-variate classification problems and random forests
# seem to improve on the decision-tree algorithm in terms of avoiding
# overfitting.
...
predict0 = predict(modFit1,train_cv)
confusionMatrix(classe_train_cv,predict0)
